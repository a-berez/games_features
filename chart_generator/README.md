# Генератор графиков взятий (chart_generator)

Скрипт генерирует HTML-страницы по турниру: интерактивный график взятий, (опционально) replay-таблицу, шахматку и страницу-оглавление `index.html`.

## Описание

Скрипт создает интерактивные графики и таблицы, показывающие, как команды продвигались по дистанции турнира, а также кратко приводит дополнительную статистику:
...
- `-t`, `--table` — создание HTML-файла с интерактивной таблицей Replay Table (+ CSV) (по умолчанию создаётся)
- `--no-table` — не создавать интерактивную таблицу
...
- `-o`, `--output` — имя каталога для выходных файлов в текущей директории (по умолчанию ID турнира или имя файла)
- `--no-crosstable` — не создавать шахматку (по умолчанию создаётся)
- `-tc`, `--tour_comp` — состав туров для шахматки, например `15,15,15,15,15,15`
- `--color HEX` — акцентный цвет заголовков (например `#71c0a1`)
- `-v/-vv` — уровни логирования (INFO/DEBUG)
...
#### Использование с таблицей (таблица создаётся по умолчанию)
```chart_generator.py 12345```

#### Без таблицы
```chart_generator.py 12345 --no-table```


#### Шахматка с разбиением по турам
```chart_generator.py 12345 -tc 15,15,15,15,15,15```
...
```chart_generator.py 12345 -vv```


## Структура выходных файлов

Скрипт собирает HTML-страницы в папку проекта; статика и данные лежат в подпапке `static/`.

Структура (для `-o chr26`):
```
chr26/
├── index.html
├── chr26_chart.html
├── chr26_table.html
├── chr26_crosstable.html
└── static/
    ├── style.css
    ├── scripts.js
    ├── chr26.csv
    └── chr26_crosstable.data.json
```
...
- Шахматка строится из масок команд; для табличных файлов маска собирается из данных вопросов (`1`/`0`/`X`)
...
- При генерации интерактивных таблиц создаётся csv-файл, который необходим для работы

## История версий

### v0.2.0

- Добавлена генерация интерактивных таблиц с помощью библиотеки [Replay Table](https://github.com/antoniokov/replay-table)
...
- Создан chart_generator

## Дополнительная информация

Используется библиотека [Replay Table](https://github.com/antoniokov/replay-table) от [Антона Иокова](https://github.com/antoniokov/).
